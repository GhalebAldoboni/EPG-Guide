...
def collect_guide(
    *, delay: float = 0.75, workers: int = 2
    ...
    discovered = apply_subscription_aliases(
        apply_aliases(discover_channels(fetch_text(INDEX_URL, limiter), BASE_URL)),
    ...
    def parse_one(channel: Channel) -> tuple[Channel, tuple[Programme, ...]]:
        ...
        return parse_channel_page(
            html, channel, scrape_date.year, reference_date=scrape_date
        )
...
def validate_guide(
    ...
    *,
    ...
    near_now = any(
        start.astimezone(DUBAI) <= now + timedelta(hours=6)
        and stop.astimezone(DUBAI) >= now - timedelta(hours=6)
        for start, stop in zip(starts, stops)
    )
    if earliest > now + timedelta(hours=6) or latest < now or not near_now:
        raise SourceParseError("guide does not cover the current broadcast window")
